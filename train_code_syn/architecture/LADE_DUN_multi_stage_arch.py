...
class TridentTransformer(nn.Module):
    def __init__(self, 
        ...
        self.add_ss = nn.Sequential(nn.Conv2d(dim,dim,1,1,0),
                                nn.Identity() if self.opt.train_stage==1 else nn.GELU()
                                 )  
        if self.split_ch>1:
...
class TridentTransformer_simple(nn.Module):
    def __init__(self, 
                 opt,
                 dim, 
                 num_heads, 
    ...
    def forward(self, x,prior=None):
        ...
        spectral_identity = x_spec
        spectral_fea,_  = self.spectral_branch(x_spec, spatial_interaction)
        ...
        if self.split_ch>1:
            fea = torch.concat([spatial_fea,spectral_fea],dim=1)
        else:
...
class DUN_Denoiser_simple(nn.Module):
    def __init__(self, opt,use_prior=False):
        super().__init__()
        self.use_prior = use_prior
        ...
        TridentTransformer_simple(opt = opt, dim = opt.dim * 2 ** 0, num_heads = 2 ** 0,
                          use_prior=self.use_prior,use_ch_split=False),
        TridentTransformer_simple(opt = opt, dim = opt.dim * 2 ** 1, num_heads = 2 ** 1,
                          use_prior=self.use_prior,use_ch_split=True),
                      
        ])

        self.BottleNeck = TridentTransformer_simple(opt = opt, dim = opt.dim * 2 ** 2, num_heads = 2 ** 2,
                                            use_prior=self.use_prior,use_ch_split=True)
...
class LADE_DUN(nn.Module):
    def __init__(self, opt):
        ...
        self.head_PM = DUN_Denoiser_simple(opt,use_prior=use_prior_flag)
        ...
        self.body_PM = nn.ModuleList([
            DUN_Denoiser_simple(opt,use_prior=use_prior_flag) for _ in range(opt.stage - 2)
        ]) if not opt.body_share_params else DUN_Denoiser_simple(opt,use_prior=use_prior_flag)
