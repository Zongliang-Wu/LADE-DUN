...
class TridentTransformer(nn.Module):
    def __init__(self, 
        ...
        self.add = nn.Sequential(nn.Conv2d(3*dim//self.split_ch,dim,1,1,0),
...
from .latent_encoder_arch import latent_encoder_gelu
...
class LADE_DUN(nn.Module):
    def __init__(self, opt):
        ...
        if opt.train_phase != 3:
            self.gt_le = latent_encoder_gelu()


        self.net_le_dm = latent_encoder_gelu(in_chans=28)
    ...
    def forward(self, y, Phi,gt=None):
        ...
        if self.train_phase==1 and prior_z is not None:
            prior = prior_z
        else: 
